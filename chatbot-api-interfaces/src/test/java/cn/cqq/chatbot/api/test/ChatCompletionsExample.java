...
public class ChatCompletionsExample {
    ...
    public static void main(String[] args) {
        ...
        final List<ChatCompletionContentPart> multiPartsNew = new ArrayList<>();
        multiPartsNew.add(ChatCompletionContentPart.builder().type("image_url").imageUrl(
              new ChatCompletionContentPart.ChatCompletionContentPartImageURL(
                  "https://ark-project.tos-cn-beijing.ivolces.com/images/view.jpeg"
              )
        ).build());
        multiPartsNew.add(ChatCompletionContentPart.builder().type("text").text(
                "这是哪里？"
        ).build());

        final ChatMessage userMessageNew = ChatMessage.builder().role(ChatMessageRole.USER)
                .multiContent(multiPartsNew).build();
        messages.add(userMessageNew);
